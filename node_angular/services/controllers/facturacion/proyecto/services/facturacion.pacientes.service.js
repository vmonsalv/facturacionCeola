    ...
    try {
        if (validador.validarCampo(JSON.parse(request.body.arg).rut, "rut")) {
            serviceCaller.getPaciente(JSON.parse(request.body.arg), function (data, error) {
                if (error == null) {
                    if(data.hasOwnProperty("ERR_MSSG")){
            ...
            response.status(200).json(reply.ok("Error en los datos ingresados"));
        }
    } catch (e) {
        response.status(200).json(reply.fatal(e.message));
    }
...
function newPaciente(request, response) {
    try {
        ...
        if (validador.validarCampo(rut, "rut")) {
            serviceCaller.newPaciente(JSON.parse(request.body.arg), function (data, error) {
                if (error == null) {
                    response.status(200).json(reply.ok(data));
                } else {
                    response.status(200).json(reply.throwsError(error));
